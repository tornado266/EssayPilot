...
"""
    ...
    def test_admin_dashboard_has_no_manual_membership_review(self):
        script = """
import src.admin_dashboard as dashboard
...
"""
        ...
        self.assertEqual(len(app.exception), 0)
        self.assertNotIn("3 篇训练包人工核单", [item.value for item in app.subheader])
        self.assertNotIn("第一步：进入核对", [button.label for button in app.button])
        self.assertNotIn("第二步：确认批准并开通", [button.label for button in app.button])
