"""Founder-pass product rules and display helpers.
...
def entitlement_caption(payload: Mapping[str, object] | None) -> str:
    ...
    if entitlement["status"] == "expired":
        return "创始体验包已到期 · 已生成内容仍可查看"
    if entitlement["status"] in {"exhausted", "used"}:
        return "完整训练篇数已用完 · 已生成内容仍可查看"
    if entitlement["status"] == "pending":
        return "创始体验包尚未开始 · 暂时不能发起新的 AI 请求"
    if entitlement["status"] == "refunded":
        return "创始体验包已退款 · 已生成内容仍可查看"
    if entitlement["status"] == "revoked":
        return "创始体验包已停止 · 已生成内容仍可查看"
...
def action_reason_message(reason: object) -> str:
    ...
    messages = {
        "membership_required": "本功能需要先开通创始体验包。",
        "membership_inactive": "体验包当前不可用，请刷新权益或联系管理员。",
        "membership_expired": "体验包已到期；已有内容仍可查看。",
