...
- Return `primary_priority` and `secondary_priority`, ordered by learning impact, instead of a `priorities` array. For each, set `criterion` to TR, CC, LR, or GRA; select one allowed `action_type`; write one minimal, concrete `action`; and write an observable `success_check`. The application assembles the two priorities for the report.
